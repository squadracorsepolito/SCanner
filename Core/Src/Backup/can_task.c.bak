...
void canInit(FDCAN_HandleTypeDef *const hfdcan, FDCAN_FilterTypeDef *const sFilterConfig, uint32_t ActiveITs) {
...
void canTask(const void * argument) {
    ...
    canInit(network->Init.hfdcan, &(network->Init.sFilterConfig), network->Init.ActiveITs);
    ...
    while(1) {
        if(xQueueReceive(network->tx_queue, &tx_frame, portMAX_DELAY)) {
            ...
            TxHeader.DataLength = tx_frame.dlc << 16;
